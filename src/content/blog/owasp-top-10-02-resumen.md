...
### [A01 - Broken Access Control](/blog/owasp-top-10-03-a01-broken-access-control)
...
### [A02 - Security Misconfiguration](/blog/owasp-top-10-04-a02-security-misconfiguration)
...
### [A03 - Software Supply Chain Failures](/blog/owasp-top-10-05-a03-software-supply-chain-failures)
- **En la vida real:** Aceptar un dulce que te ofrece un extraño en la calle, solo porque se ve apetitoso y gratis.
- **En tu App:** Instalar ciegamente un paquete `npm` de un desconocido que inyecta código malicioso en las entrañas de tu servidor de producción.
- **Cómo mitigarlo:** Revisa a los creadores de tus dependencias, usa archivos `.lock` para fijar versiones, y no confíes ciegamente en librerías abandonadas.

### [A04 - Cryptographic Failures](/blog/owasp-top-10-06-a04-cryptographic-failures)
...
- **En tu App:** Guardar contraseñas en texto plano en la base de datos o transmitir información sensible de tus usuarios usando HTTP sin cifrar.
...
### [A05 - Insecure Design](/blog/owasp-top-10-07-a05-insecure-design)
...
- **En tu App:** Flujos de negocio mal pensados lógicamente, como permitir recuperar una contraseña respondiendo a la pregunta "¿Cuál es tu color favorito?".
- **Cómo mitigarlo:** Aplica el *Threat Modeling* (Modelado de Amenazas). Piensa sistemáticamente como un atacante desde la pizarra, antes de escribir la primera línea de código.

### [A06 - Vulnerable and Outdated Components](/blog/owasp-top-10-08-a06-vulnerable-components)
...
- **En tu App:** Usar una versión vieja de un *framework* que tiene vulnerabilidades públicas ultra conocidas (como la famosa crisis de Log4Shell).
...
### [A07 - Identification and Authentication Failures](/blog/owasp-top-10-09-a07-authentication-failures)
- **En la vida real:** Un guardia de seguridad que te deja entrar a un edificio confidencial solo porque traes una camisa del mismo color que el uniforme de empleado.
- **En tu App:** Permitir ataques de fuerza bruta interminables, secuestro de sesiones (Tokens JWT sin expiración) o inicios de sesión sin autenticación multifactor (MFA).
...
### [A08 - Software and Data Integrity Failures](/blog/owasp-top-10-10-a08-software-integrity)
- **En la vida real:** Recibir un paquete, confiar ciegamente en el logo de la caja y no revisar que alguien cortó, abrió y alteró el sello de seguridad en el camino.
...
- **Cómo mitigarlo:** Usa repositorios internos validados, verifica exhaustivamente los *hashes* de los archivos y asegura estrictamente los accesos a tu *pipeline* de DevOps.

### [A09 - Security Logging and Monitoring Failures](/blog/owasp-top-10-11-a09-security-logging)
...
- **En tu App:** Un atacante entra a tu base de datos y roba todo, pero tú te enteras seis meses después por las noticias porque no registraste el acceso.
- **Cómo mitigarlo:** Registra eventos de negocio críticos, centralízalos en un servidor inmutable externo (como Datadog) y configura alertas activas que te despierten.

### [A10 - Server-Side Request Forgery (SSRF)](/blog/owasp-top-10-12-a10-ssrf)
...
- **En tu App:** Mandar una URL maliciosa a la web para que tu servidor público funcione como un puente y ataque a tu propia red interna o a bases de datos ocultas.
- **Cómo mitigarlo:** Usa *Allow-lists* (Listas blancas) sumamente estrictas, bloquea el acceso a direcciones IP locales o de metadatos de nube, y aplica segmentación de red.
